...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.world = pydart.World(1./150., cur_path+'/../../data/skel/human_mass_limited_dof_v2_ball_mound25.skel')
        ...
        self.ref_world = pydart.World(1./150., cur_path+'/../../data/skel/human_mass_limited_dof_v2_ball_mound25.skel')
        self.ref_skel = self.ref_world.skeletons[1]
        self.ref_motion = DartSkelMotion()
        self.ref_motion.load(cur_path + '/postpost25.skmo')
        self.ref_motion.reset_root_trajectory_postpost25(self.ref_skel)
        ...
        for json_file in sorted(glob.glob(json_path + "*.json")):
            ...
            if json_file == json_path+"postpost25_0000000000096_keypoints.json":
                for i in range(10):
    ...
    def reward(self):
        ...
        skel_pos_diff[16:28] = 0.1*skel_pos_diff[16:28]
        ...
        skel_vel_diff[16:28] = 0.1 * skel_vel_diff[16:28]
        ...
        ###
        ankle_pos = self.skel.joint('j_heel_left').position_in_world_frame()

        if self.mound_contact_frame == self.current_frame:

            # mound up vector
            m_angle = np.arcsin(0.25)
            m_matrix = mm.rotX(m_angle)
            mound_vec = np.dot(m_matrix, y_axis)

            # foot vector
            knee_pos = self.skel.joint('j_shin_left').position_in_world_frame()
            foot_vec = ankle_pos - knee_pos

            contact_angle = acos(np.dot(foot_vec, mound_vec) / (np.linalg.norm(foot_vec) * np.linalg.norm(mound_vec)))
            contact_angle_diff = abs(pi - contact_angle)

            r_ca = exp_reward_term(self.w_ca, self.exp_ca, [contact_angle_diff])

            reward = (1. - self.w_ca) * reward + r_ca

            t_vector = np.array([0., 0., 18.44])
            thigh_pos = self.skel.joint('j_thigh_left').position_in_world_frame()


            u_vector = thigh_pos - knee_pos
            d_vector = ankle_pos - knee_pos
            n_vector = mm.cross(u_vector, d_vector)

            v_angle = acos(np.dot(n_vector, t_vector) / (np.linalg.norm(n_vector) * np.linalg.norm(t_vector)))
            v_angle_diff = abs(pi / 2 - v_angle)

            r_va = exp_reward_term(self.w_ca, self.exp_ca, [v_angle_diff])

            reward = (1. - self.w_ca) * reward + r_va

        if self.mound_contact_frame - 5 <= self.current_frame <= self.mound_contact_frame:
            if ankle_pos[2] >= self.prev_ankle_pos:
                reward = 0.9*reward + 0.1

        self.prev_ankle_pos = ankle_pos[2]
        ...
        if self.ball_released_frame < self.current_frame:
            ...
            # reward = reward + 0.5*r_ball_direction
            ...
            reward = 0.5*reward + 0.25*r_ball_speed + 0.25*r_ball_direction

        return reward

        # if self.current_frame == 45 :
        #     ball_dis_real = sqrt((self.target_zone_pos[0] - self.ball.com()[0])**2+(self.target_zone_pos[1] - self.ball.com()[1])**2+(self.target_zone_pos[2] - self.ball.com()[2])**2)
        #     # b_w_d = exp(-((ball_dis_real)**2))
        #     if ball_dis_real >= 0.1 :
        #         b_w_d = 1/ball_dis_real
        #     else :
        #         b_w_d = 15
        #     print('reward: ', reward)
        #     print("b_w_d: ", b_w_d)
        #     reward = reward + b_w_d / 10
