...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.world = pydart.World(1./150., cur_path+'/../../data/skel/human_mass_limited_dof_v2_ball.skel')
        self.world.control_skel = self.world.skeletons[1]
        self.skel = self.world.skeletons[1]
        self.Kp, self.Kd = 600., 49.

        self.ref_world = pydart.World(1./150., cur_path+'/../../data/skel/human_mass_limited_dof_v2_ball.skel')
        ...
        # draw box
        # self.box_height = 0.254
        # self.box_size = [1.524, self.box_height, 0.8636]
        #
        # self.world.skeletons[0].add_box("box", self.box_size, [0.55, 0.25, 0.0])
        # self.world.skeletons[0].body("box").set_collidable(True)
        #
        # self.box_pos = []
        # self.box_pos.append(np.array([0., 0.5 * self.box_height, 0.]))



        self.height_hat_list = [0.254, 0.254, 0.1574, 0.0964, 0.5]



        # draw box
        # add box([name], [size], [color])
        self.box0_size = [1.524, self.height_hat_list[0], 0.8636]
        self.box1_size = [0.6096, self.height_hat_list[1], 0.1524]
        self.box2_size = [1.524, self.height_hat_list[2], 1.8288]
        self.box3_size = [1.524, self.height_hat_list[3], 1.83135]
        #self.box4_size = [3.0, self.height_hat_list[4], 0.3]

        self.world.skeletons[0].add_box("box0", self.box0_size, [0.55, 0.25, 0.0])
        self.world.skeletons[0].add_box("box1", self.box1_size, [1.0, 1.0, 1.0])
        self.world.skeletons[0].add_box("box2", self.box2_size, [0.55, 0.25, 0.0])
        self.world.skeletons[0].add_box("box3", self.box3_size, [0.55, 0.25, 0.0])
        #self.world.skeletons[0].add_box("box4", self.box4_size, [1, 1, 1])

        self.box_pos = []
        self.box_pos.append(np.array([0., 0.5 * self.height_hat_list[0], -0.2]))
        self.box_pos.append(np.array([0., 0.5 * self.height_hat_list[1], 0.0]))
        self.box_pos.append(np.array([0., 0.5 * self.height_hat_list[2], 1.1462]))
        self.box_pos.append(np.array([0., 1.635 * self.height_hat_list[3], 1.1]))
        #self.box_pos.append(np.array([0., 0.5 * self.height_hat_list[4], 8.0]))

        self.box_rot = []
        self.box_rot.append(mm.logSO3(mm.rotX(mm.deg2Rad(3.024))))
