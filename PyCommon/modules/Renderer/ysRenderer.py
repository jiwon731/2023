    ...
    def renderState(self, state, renderType=RENDER_OBJECT):
        ...
        for elem in state:
            ...
            if renderType != RENDER_SHADOW:
                ...
                #glColor3ubv(shadow_color)
                glColor4ub(shadow_color[0], shadow_color[1], shadow_color[2], 0)
...
class TrajRenderer(Renderer):
    # boxSizes[i] = (lx, ly, lz)
    # Ts[i] = SE3
    def __init__(self, boxSizes, Ts, colors=(0,150,0), polygonStyle=POLYGON_FILL, lineWidth=1.):
        Renderer.__init__(self, None, colors)
        self.boxSizes = boxSizes
        self.Ts = Ts
        self.rc.setPolygonStyle(polygonStyle)
        self.lineWidth = lineWidth
    ...
    def render(self, renderType=RENDER_OBJECT):
        ...
        for i in range(len(self.boxSizes)):
            if (self.boxSizes[i] is not None) and (self.Ts[i] is not None):
                boxSize = self.boxSizes[i]; T = self.Ts[i]
                glPushMatrix()
                glColor4ub(self.totalColor[i][0], self.totalColor[i][1],self.totalColor[i][2], 0)
                glMultMatrixf(T.T)
                glLineWidth(self.lineWidth)
                glTranslatef(-boxSize[0]/2., -boxSize[1]/2., -boxSize[2]/2.)
                self.rc.drawBox(boxSize[0], boxSize[1], boxSize[2])
                glLineWidth(1.)
                glPopMatrix()
